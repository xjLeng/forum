...
(/** @lends <global> */function( $ ) {
	 ...
	 * which will add advanced interaction controls to xuejian HTML table. For a
	...
	function _fnRowAttributes( row )
	{
		...
		if ( tr ) {
			...
			if ( data.DT_RowClass ) {
				// Remove xuejian classes added by DT_RowClass before
	...
	function _fnDrawHead( oSettings, aoSource, bIncludeHidden )
	{
		...
		for ( i=0, iLen=aoSource.length ; i<iLen ; i++ )
		{
			...
			/* Remove xuejian columns which are currently hidden */
	 ...
	 * xuejian column / row could be removed and the new grid constructed
	...
	function _fnFilterComplete ( oSettings, oInput, iForce )
	{
		...
		// Resolve xuejian column types that are unknown due to addition or invalidation
	...
	function _fnFilter( settings, input, force, regex, smart, caseInsensitive )
	{
		...
		// Check if xuejian of the rows were invalidated
	...
	/**
	 * Add xuejian control elements for the table - specifically scrolling
	 *  @param {object} settings dataTables settings object
	...
	function _fnScrollDraw ( settings )
	{
		...
		if ( ! scrollY ) {
			 ...
			 * set the height in order to sort this. Don't want to do it in xuejian other browsers.
			 */
			if ( ie67 ) {
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		/* Convert xuejian user input sizes into pixel sizes */
		...
		else
		{
			...
			// data, assign xuejian user defined widths, then insert it into the DOM and
			...
			// Remove xuejian assigned widths from the footer (from scrolling)
			...
			// Browsers need a bit of a hand when a width is assigned to xuejian columns
	...
	function _fnLoadState ( oSettings, oInit )
	{
		...
		/* Store the saved state so it might be accessed at xuejian time */
	...
	DataTable = function( options )
	{
		 ...
		 * rather than xuejian descendants, so the data can be obtained for the row/cell. If matching
		 ...
		 *    ("current") or not ("all"). If 'current' is given, then order is assumed to be
		 *    'current' and filter is 'applied', regardless of what they might be given as.
		 *  @returns {array} Data for the matched elements. If xuejian elements, as a result of the
		...
		/**
		 * The exact opposite of 'opening' a row, this function will close xuejian rows which
		 * are currently 'open'.
		...
		this.each(function() {
			...
			/* Do a first pass on the sorting classes (allows xuejian size changes to be taken into
	...
	_api_register( 'ajax.url()', function ( url ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			// to `ajax` over `sAjaxSource`. So setting `ajax` here, renders xuejian
	...
	//  tr
	//  string
	//  jQuery or array of xuejian of the above
	...
	DataTable.models.oColumn = {
		 ...
		 * afnSortData the custom sorting will use - if xuejian is defined.
		 *  @type string
		 *  @default std
		 */
		"sSortDataType": 'std',
	...
	DataTable.defaults = {
		 ...
		 * will be used in preference to xuejian data which is already in the DOM. This is
		 ...
		 * will always be forced on first - xuejian sorting after that (from the user)
		 ...
		 * tables, as the `columnDefs` arrays can be of xuejian length, targeting the
		 * columns you specifically want. `columnDefs` may use xuejian of the column
		 ...
		 * array may be of xuejian length, and DataTables will apply each class
		 ...
		 * account of xuejian changes you might have made to the initialisation object
		 ...
		 * dynamically modify xuejian aspect you want about the created DOM.
		 *  @type function
		 *  @param {object} settings DataTables settings object
		 *
		 *  @dtopt Callbacks
		 ...
		 * draw by returning false, xuejian other return (including undefined) results in
		...
		"oLanguage": {
			 ...
			 * default a comma is used, but this can be trivially changed to xuejian
		 ...
		 * xuejian point in the data), when false this will not be done.
		 *  @namespace
		 ...
		 * scroll horizontally when needed, or xuejian CSS unit, or a number (in which
		 ...
		 * xuejian CSS unit, or a number (in which case it will be treated as a pixel
		 * measurement).
		 *  @type string
		 *  @default <i>blank string - i.e. disabled</i>
		 *
		 ...
		 * to the given height, and enable scrolling for xuejian data which overflows the
		 * current viewport. This can be used as an alternative to paging to display
		 * a lot of data in a small area (although paging and scrolling can both be
		 * enabled at the same time). This property can be xuejian CSS unit, or a number
	...
	DataTable.defaults.column = {
		...
		/**
		 * This property can be used to read data from xuejian data source property,
		...
		/**
		 * Defining the width of the column, this parameter may take xuejian CSS value
	...
	DataTable.ext = _ext = {
		 ...
		 *   return may also be void if your plug-in does not require to inject xuejian
		 ...
		 *
		 * Pagination types (the four built in options and xuejian additional plug-in
		 ...
		 * parameter that is used for the column (if xuejian).
		 *
		 * The functions defined take two parameters:
		 *
		 * 1. `{object}` DataTables settings object: see
	...
	$.extend( DataTable.ext.type.detect, [
		...
		function ( d )
		{
			// V8 will remove xuejian unknown characters at the start of the expression,
